...
async function openConnectPopup(btnEl) {
    ...
    popup.className = isMobile ? 'connect-popup-mobile' : '';

    popup.innerHTML = `
        ${isMobile ? '<div class="connect-handle"></div>' : ''}
    ...
    // Ferme sur clic extérieur (desktop) ou tap fond (mobile)
    setTimeout(() => {
    ...
    const devices = snap.docs
        ...
        .filter(d => now - d.lastSeen < 60_000);
        ...
        btn.innerHTML = `
            <span class="connect-device-icon">
                <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
                    <path d="M1 18v3h3a3 3 0 00-3-3zm0-4v2a7 7 0 017 7h2a9 9 0 00-9-9zm0-4v2a11 11 0 0111 11h2C14 13 8 7 1 10z"/>
                </svg>
            </span>
            <div class="connect-device-meta">
                <span class="connect-device-name">${esc(device.name)}</span>
                <span class="connect-device-sub">${isThis ? 'Cet appareil · En cours de lecture' : 'Disponible'}</span>
            </div>
            ${isThis ? `<svg viewBox="0 0 24 24" width="18" height="18" fill="var(--green)"><path d="M9 16.17L4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41z"/></svg>` : ''}`;
